...
async function createNew () {
    ...
    if (!checkLatest || checkLatest?.dateStart < dateNow) {
        ...
        let create = await transactionModel.create({
            ...
            auto: setting?.auto,
            status: "pending",
            dateStart: moment().startOf('minute').add((setting?.timePerRound) - setting?.timeAdd, "seconds"),
            dateEnd: moment().startOf('minute').add((setting?.timePerRound * 2) - setting?.timeAdd, "seconds"),
        })
